...
#define MINIMUM_DISTANCE 0.00000001
#define CLUSTER_MINIMUM_DISTANCE 0.5
...
vector<vector<double> > MeanShift::meanshift(const vector<vector<double> > & points, double kernel_bandwidth){
    ...
    do {
        ...
        for(int i=0; i<shifted_points.size(); i++){
            if (!stop_moving[i]) {
                ...
                if(shift_distance <= MINIMUM_DISTANCE) {
                    stop_moving[i] = true;
                }
    ...
    } while (max_shift_distance > MINIMUM_DISTANCE);
    ...
    const vector<vector<double> > & shifted_points)
{
    ...
    for (int i = 0; i < shifted_points.size(); i++) {
        ...
        for (; c < clusters.size(); c++) {
            if (euclidean_distance(shifted_points[i], clusters[c].mode) <= CLUSTER_MINIMUM_DISTANCE) {
                break;
            }
        }
